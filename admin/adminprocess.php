...
class AdminProcess {
    ...
    function AdminProcess() {
        ...
        /* Admin submitted delete comment form */ else if (isset($_POST['delcomment'])) {
            $this->procDeleteComment();
        }
        /* Admin submitted delete comment form */ else if (isset($_POST['delcomment_answer'])) {
            $this->procDeleteCommentAnswer();
        }
    ...
    function procDeleteArticle() {
     ...
     /**
     * procDeleteComment - If the submitted comment id is correct,
    ...
    function procDeleteComment() {
        ...
        /* Delete comment from database */ else {
            $id = $_POST['id'];
            $q = "DELETE FROM komentaras WHERE id = '$id'";
            $database->query($q);
            header("Location: " . $session->referrer);
        }
    }
     /**
     * procDeleteCommentAnswer - If the submitted comment id is correct,
     * the comment is deleted from the database.
     */
    function procDeleteCommentAnswer() {
        global $session, $database, $form;
        /* Errors exist, have user correct them */
        if ($form->num_errors > 0 && false) {
            $_SESSION['value_array'] = $_POST;
        ...
        /* Delete comment from database */ else {
            $id = $_POST['id'];
            $q = "DELETE FROM komentaru_ats WHERE id = '$id'";
            $database->query($q);
            header("Location: " . $session->referrer);
        }
    }
